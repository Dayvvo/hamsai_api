# 518849455 (2024-03-16T22:29:35.693Z)

{
  "update_id": 518849455,
  "message": {
    "message_id": 462,
    "from": {
      "id": 5258418651,
      "is_bot": false,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "language_code": "en"
    },
    "chat": {
      "id": 5258418651,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "type": "private"
    },
    "date": 1710628176,
    "text": "/play",
    "entities": [
      {
        "offset": 0,
        "length": 5,
        "type": "bot_command"
      }
    ]
  }
}

# 518849454 (2024-03-16T22:29:11.663Z)

{
  "update_id": 518849454,
  "callback_query": {
    "id": "4137992064064571671",
    "from": {
      "id": 5258418651,
      "is_bot": false,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "language_code": "en"
    },
    "message": {
      "message_id": 460,
      "from": {
        "id": 7064107854,
        "is_bot": true,
        "first_name": "hamsai_bot",
        "username": "hamsai_bot"
      },
      "chat": {
        "id": 5258418651,
        "first_name": "Goran",
        "last_name": "Gutovic",
        "username": "gg97999",
        "type": "private"
      },
      "date": 1710628150,
      "text": "Welcome to HamsterAI Bot - your go-to destination for exhilarating hamster racing bets on Telegram! 🐹 Ready to place your bets? Click \"Play Now!\" to join the action. Experience the thrill of betting on your favorite hamsters in real-time. 🚀\n\nDive into the fun with two betting pools - bet on your champion hamster and join the excitement. All bets are pooled together, creating a big pot for the winners. 🏆\n\nGot questions or need assistance? Our dedicated support team is here to help you 24/7.\n\n🎰 Join our community, place your bets, and watch as the winning hamster brings you a fortune. Each race features 3 players and 3 pools - the winner takes the entire pot, minus a small tax, evenly split based on the bet amount. Don’t miss out, start betting now and may the best hamster win! 🐹💰",
      "reply_markup": {
        "inline_keyboard": [
          [
            {
              "text": "Commands ❓",
              "callback_data": "commands"
            }
          ]
        ]
      }
    },
    "chat_instance": "2696435764436939867",
    "data": "commands"
  }
}

# 518849453 (2024-03-16T22:29:08.857Z)

{
  "update_id": 518849453,
  "message": {
    "message_id": 459,
    "from": {
      "id": 5258418651,
      "is_bot": false,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "language_code": "en"
    },
    "chat": {
      "id": 5258418651,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "type": "private"
    },
    "date": 1710628149,
    "text": "/start",
    "entities": [
      {
        "offset": 0,
        "length": 6,
        "type": "bot_command"
      }
    ]
  }
}

# 518849408 (2024-03-16T00:50:28.760Z)

{
  "update_id": 518849408,
  "message": {
    "message_id": 399,
    "from": {
      "id": 5258418651,
      "is_bot": false,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "language_code": "en"
    },
    "chat": {
      "id": 5258418651,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "type": "private"
    },
    "date": 1710550229,
    "text": "/deposit",
    "entities": [
      {
        "offset": 0,
        "length": 8,
        "type": "bot_command"
      }
    ]
  }
}

# 518849407 (2024-03-16T00:50:22.727Z)

{
  "update_id": 518849407,
  "callback_query": {
    "id": "4137992064864093629",
    "from": {
      "id": 5258418651,
      "is_bot": false,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "language_code": "en"
    },
    "message": {
      "message_id": 397,
      "from": {
        "id": 7064107854,
        "is_bot": true,
        "first_name": "hamsai_bot",
        "username": "hamsai_bot"
      },
      "chat": {
        "id": 5258418651,
        "first_name": "Goran",
        "last_name": "Gutovic",
        "username": "gg97999",
        "type": "private"
      },
      "date": 1710550222,
      "text": "You've chosen Pool 2. Now, choose your bet amount.",
      "reply_markup": {
        "inline_keyboard": [
          [
            {
              "text": "0.05",
              "callback_data": "bet_2_0.05"
            },
            {
              "text": "0.1",
              "callback_data": "bet_2_0.1"
            },
            {
              "text": "0.5",
              "callback_data": "bet_2_0.5"
            },
            {
              "text": "1",
              "callback_data": "bet_2_1"
            },
            {
              "text": "1.5",
              "callback_data": "bet_2_1.5"
            },
            {
              "text": "2",
              "callback_data": "bet_2_2"
            }
          ]
        ]
      }
    },
    "chat_instance": "2696435764436939867",
    "data": "bet_2_0.1"
  }
}

# 518849406 (2024-03-16T00:50:21.900Z)

{
  "update_id": 518849406,
  "callback_query": {
    "id": "4137992062878801183",
    "from": {
      "id": 5258418651,
      "is_bot": false,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "language_code": "en"
    },
    "message": {
      "message_id": 396,
      "from": {
        "id": 7064107854,
        "is_bot": true,
        "first_name": "hamsai_bot",
        "username": "hamsai_bot"
      },
      "chat": {
        "id": 5258418651,
        "first_name": "Goran",
        "last_name": "Gutovic",
        "username": "gg97999",
        "type": "private"
      },
      "date": 1710550222,
      "text": "Choose a pool to bet in:",
      "reply_markup": {
        "inline_keyboard": [
          [
            {
              "text": "Pool 1",
              "callback_data": "pool_1"
            },
            {
              "text": "Pool 2",
              "callback_data": "pool_2"
            },
            {
              "text": "Pool 3",
              "callback_data": "pool_3"
            },
            {
              "text": "Pool 4",
              "callback_data": "pool_4"
            },
            {
              "text": "Pool 5",
              "callback_data": "pool_5"
            }
          ]
        ]
      }
    },
    "chat_instance": "2696435764436939867",
    "data": "pool_2"
  }
}

# 518849405 (2024-03-16T00:50:20.747Z)

{
  "update_id": 518849405,
  "message": {
    "message_id": 395,
    "from": {
      "id": 5258418651,
      "is_bot": false,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "language_code": "en"
    },
    "chat": {
      "id": 5258418651,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "type": "private"
    },
    "date": 1710550221,
    "text": "/play",
    "entities": [
      {
        "offset": 0,
        "length": 5,
        "type": "bot_command"
      }
    ]
  }
}

# 518849404 (2024-03-16T00:50:13.917Z)

{
  "update_id": 518849404,
  "message": {
    "message_id": 392,
    "from": {
      "id": 5258418651,
      "is_bot": false,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "language_code": "en"
    },
    "chat": {
      "id": 5258418651,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "type": "private"
    },
    "date": 1710550214,
    "text": "/new_race 30",
    "entities": [
      {
        "offset": 0,
        "length": 9,
        "type": "bot_command"
      }
    ]
  }
}

# 518849403 (2024-03-16T00:50:10.922Z)

{
  "update_id": 518849403,
  "message": {
    "message_id": 390,
    "from": {
      "id": 5258418651,
      "is_bot": false,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "language_code": "en"
    },
    "chat": {
      "id": 5258418651,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "type": "private"
    },
    "date": 1710550211,
    "text": "/new_race",
    "entities": [
      {
        "offset": 0,
        "length": 9,
        "type": "bot_command"
      }
    ]
  }
}

# 518849402 (2024-03-16T00:50:06.109Z)

{
  "update_id": 518849402,
  "callback_query": {
    "id": "4137992061578519567",
    "from": {
      "id": 5258418651,
      "is_bot": false,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "language_code": "en"
    },
    "message": {
      "message_id": 385,
      "from": {
        "id": 7064107854,
        "is_bot": true,
        "first_name": "hamsai_bot",
        "username": "hamsai_bot"
      },
      "chat": {
        "id": 5258418651,
        "first_name": "Goran",
        "last_name": "Gutovic",
        "username": "gg97999",
        "type": "private"
      },
      "date": 1710550154,
      "text": "You've chosen Pool 1. Now, choose your bet amount.",
      "reply_markup": {
        "inline_keyboard": [
          [
            {
              "text": "0.05",
              "callback_data": "bet_1_0.05"
            },
            {
              "text": "0.1",
              "callback_data": "bet_1_0.1"
            },
            {
              "text": "0.5",
              "callback_data": "bet_1_0.5"
            },
            {
              "text": "1",
              "callback_data": "bet_1_1"
            },
            {
              "text": "1.5",
              "callback_data": "bet_1_1.5"
            },
            {
              "text": "2",
              "callback_data": "bet_1_2"
            }
          ]
        ]
      }
    },
    "chat_instance": "2696435764436939867",
    "data": "bet_1_0.1"
  }
}

# 518849401 (2024-03-16T00:49:19.640Z)

{
  "update_id": 518849401,
  "message": {
    "message_id": 387,
    "from": {
      "id": 5258418651,
      "is_bot": false,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "language_code": "en"
    },
    "chat": {
      "id": 5258418651,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "type": "private"
    },
    "date": 1710550160,
    "text": "/balance",
    "entities": [
      {
        "offset": 0,
        "length": 8,
        "type": "bot_command"
      }
    ]
  }
}

# 518849400 (2024-03-16T00:49:14.525Z)

{
  "update_id": 518849400,
  "callback_query": {
    "id": "4137992062030376627",
    "from": {
      "id": 5258418651,
      "is_bot": false,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "language_code": "en"
    },
    "message": {
      "message_id": 385,
      "from": {
        "id": 7064107854,
        "is_bot": true,
        "first_name": "hamsai_bot",
        "username": "hamsai_bot"
      },
      "chat": {
        "id": 5258418651,
        "first_name": "Goran",
        "last_name": "Gutovic",
        "username": "gg97999",
        "type": "private"
      },
      "date": 1710550154,
      "text": "You've chosen Pool 1. Now, choose your bet amount.",
      "reply_markup": {
        "inline_keyboard": [
          [
            {
              "text": "0.05",
              "callback_data": "bet_1_0.05"
            },
            {
              "text": "0.1",
              "callback_data": "bet_1_0.1"
            },
            {
              "text": "0.5",
              "callback_data": "bet_1_0.5"
            },
            {
              "text": "1",
              "callback_data": "bet_1_1"
            },
            {
              "text": "1.5",
              "callback_data": "bet_1_1.5"
            },
            {
              "text": "2",
              "callback_data": "bet_1_2"
            }
          ]
        ]
      }
    },
    "chat_instance": "2696435764436939867",
    "data": "bet_1_0.05"
  }
}

# 518849399 (2024-03-16T00:49:13.679Z)

{
  "update_id": 518849399,
  "callback_query": {
    "id": "4137992061286937262",
    "from": {
      "id": 5258418651,
      "is_bot": false,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "language_code": "en"
    },
    "message": {
      "message_id": 384,
      "from": {
        "id": 7064107854,
        "is_bot": true,
        "first_name": "hamsai_bot",
        "username": "hamsai_bot"
      },
      "chat": {
        "id": 5258418651,
        "first_name": "Goran",
        "last_name": "Gutovic",
        "username": "gg97999",
        "type": "private"
      },
      "date": 1710550153,
      "text": "Choose a pool to bet in:",
      "reply_markup": {
        "inline_keyboard": [
          [
            {
              "text": "Pool 1",
              "callback_data": "pool_1"
            },
            {
              "text": "Pool 2",
              "callback_data": "pool_2"
            },
            {
              "text": "Pool 3",
              "callback_data": "pool_3"
            },
            {
              "text": "Pool 4",
              "callback_data": "pool_4"
            },
            {
              "text": "Pool 5",
              "callback_data": "pool_5"
            }
          ]
        ]
      }
    },
    "chat_instance": "2696435764436939867",
    "data": "pool_1"
  }
}

# 518849398 (2024-03-16T00:49:12.465Z)

{
  "update_id": 518849398,
  "message": {
    "message_id": 383,
    "from": {
      "id": 5258418651,
      "is_bot": false,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "language_code": "en"
    },
    "chat": {
      "id": 5258418651,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "type": "private"
    },
    "date": 1710550153,
    "text": "/play",
    "entities": [
      {
        "offset": 0,
        "length": 5,
        "type": "bot_command"
      }
    ]
  }
}

# 518849397 (2024-03-16T00:49:10.744Z)

{
  "update_id": 518849397,
  "callback_query": {
    "id": "4137992063048014676",
    "from": {
      "id": 5258418651,
      "is_bot": false,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "language_code": "en"
    },
    "message": {
      "message_id": 381,
      "from": {
        "id": 7064107854,
        "is_bot": true,
        "first_name": "hamsai_bot",
        "username": "hamsai_bot"
      },
      "chat": {
        "id": 5258418651,
        "first_name": "Goran",
        "last_name": "Gutovic",
        "username": "gg97999",
        "type": "private"
      },
      "date": 1710550150,
      "text": "Welcome to HamsterAI Bot - your go-to destination for exhilarating hamster racing bets on Telegram! 🐹 Ready to place your bets? Click \"Play Now!\" to join the action. Experience the thrill of betting on your favorite hamsters in real-time. 🚀\n\nDive into the fun with two betting pools - bet on your champion hamster and join the excitement. All bets are pooled together, creating a big pot for the winners. 🏆\n\nGot questions or need assistance? Our dedicated support team is here to help you 24/7.\n\n🎰 Join our community, place your bets, and watch as the winning hamster brings you a fortune. Each race features 3 players and 3 pools - the winner takes the entire pot, minus a small tax, evenly split based on the bet amount. Don’t miss out, start betting now and may the best hamster win! 🐹💰",
      "reply_markup": {
        "inline_keyboard": [
          [
            {
              "text": "🎮 Play Now!",
              "callback_data": "play"
            }
          ],
          [
            {
              "text": "👥 Show pools",
              "callback_data": "availablePools"
            }
          ],
          [
            {
              "text": "❓ Help",
              "callback_data": "help"
            },
            {
              "text": "⚙️ Settings",
              "callback_data": "settings"
            }
          ]
        ]
      }
    },
    "chat_instance": "2696435764436939867",
    "data": "play"
  }
}

# 518849396 (2024-03-16T00:49:09.389Z)

{
  "update_id": 518849396,
  "message": {
    "message_id": 380,
    "from": {
      "id": 5258418651,
      "is_bot": false,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "language_code": "en"
    },
    "chat": {
      "id": 5258418651,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "type": "private"
    },
    "date": 1710550150,
    "text": "/start",
    "entities": [
      {
        "offset": 0,
        "length": 6,
        "type": "bot_command"
      }
    ]
  }
}

# 518849395 (2024-03-16T00:49:04.913Z)

{
  "update_id": 518849395,
  "message": {
    "message_id": 377,
    "from": {
      "id": 5258418651,
      "is_bot": false,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "language_code": "en"
    },
    "chat": {
      "id": 5258418651,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "type": "private"
    },
    "date": 1710550145,
    "text": "/new_race 30",
    "entities": [
      {
        "offset": 0,
        "length": 9,
        "type": "bot_command"
      }
    ]
  }
}

# 518849394 (2024-03-16T00:49:01.471Z)

{
  "update_id": 518849394,
  "message": {
    "message_id": 375,
    "from": {
      "id": 5258418651,
      "is_bot": false,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "language_code": "en"
    },
    "chat": {
      "id": 5258418651,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "type": "private"
    },
    "date": 1710550142,
    "text": "/new_race",
    "entities": [
      {
        "offset": 0,
        "length": 9,
        "type": "bot_command"
      }
    ]
  }
}

# 518849393 (2024-03-16T00:45:53.768Z)

{
  "update_id": 518849393,
  "message": {
    "message_id": 373,
    "from": {
      "id": 5258418651,
      "is_bot": false,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "language_code": "en"
    },
    "chat": {
      "id": 5258418651,
      "first_name": "Goran",
      "last_name": "Gutovic",
      "username": "gg97999",
      "type": "private"
    },
    "date": 1710549954,
    "text": "/balance",
    "entities": [
      {
        "offset": 0,
        "length": 8,
        "type": "bot_command"
      }
    ]
  }
}